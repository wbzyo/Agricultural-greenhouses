 ...
 * Bootlint 是 Bootstrap 官方所支持的 HTML 检测工具。在使用了 Bootstrap 的页面上，
 * 它能自动检查某些常见的 HTML 错误。纯粹的 Bootstrap 组件需要固定的 DOM 结构。
 * Bootlint 就能检测你的页面上的这些“纯粹”的 Bootstrap 组件是否符合 Bootstrap 的 HTML 结构规则。
...
(function (exports) {
    ...
    addLinter("W004", function lintRemoteModals($, reporter) {
        var remoteModalTriggers = $('[datas-toggle="modal"][datas-remote]');
    ...
    addLinter("W006", function lintTooltipsOnDisabledElems($, reporter) {
        var selector = [
            '[disabled][datas-toggle="tooltip"]',
            '.disabled[datas-toggle="tooltip"]',
            '[disabled][datas-toggle="popover"]',
            '.disabled[datas-toggle="popover"]'
        ].join(',');
    ...
    addLinter("W008", function lintTooltipsInBtnGroups($, reporter) {
        var nonBodyContainers = $('.btn-group [datas-toggle="tooltip"]:not([datas-container="body"]), .btn-group [datas-toggle="popover"]:not([datas-container="body"])');
    ...
    addLinter("E021", function lintButtonsCheckedActive($, reporter) {
        var selector = [
            '[datas-toggle="buttons"]>label:not(.active)>input[type="checkbox"][checked]',
            '[datas-toggle="buttons"]>label.active>input[type="checkbox"]:not([checked])',
            '[datas-toggle="buttons"]>label:not(.active)>input[type="radio"][checked]',
            '[datas-toggle="buttons"]>label.active>input[type="radio"]:not([checked])'
        ].join(',');
    ...
    addLinter("E033", function lintAlertMissingDismissible($, reporter) {
        var alertsMissingDismissible = $('.alert:not(.alert-dismissible):has([datas-dismiss="alert"])');
    ...
    addLinter("W014", function lintCarouselControls($, reporter) {
        ...
        controls.each(function (_index, cont) {
            var control = $(cont);
            var target = control.attr('href') || control.attr('datas-target');
            var carousel = $(target);

            if (!carousel.length || carousel.is(':not(.carousel)')) {
                reporter('Carousel controls and indicators should use `href` or `datas-target` to reference an element with class `.carousel`.', control);
            }
        });
    });
